...
When looking at vector, we see that we have the rankings, a period, a space, and a team name. We can't just split on a space or even a ". ", so we need to get a little bit crafty with our regular expressions.
...
```{r}
almostCompleteData = mutate(almostCompleteData, 
                           joiningName = gsub("\\.|\\(|\\)", "", Team), 
                           joiningName = gsub("-", " ", joiningName))
```
...
```{r}
masseyComparison = mutate(masseyComparison, 
                          joiningName = gsub("\\.|\\(|\\)", "", Team), 
                          joiningName = gsub("-", " ", joiningName))

test = stringdist_left_join(almostCompleteData, masseyComparison, by = "joiningName", 
